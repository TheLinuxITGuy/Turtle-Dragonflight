--ReputationWatchBar
--Interface\\AddOns\\Turtle-Dragonflight\\img\\TentMax.tga

--[[
This works, just needs status bar:
-- Hide the original ReputationWatchBar
ReputationWatchBar:SetAlpha(0)

--MAIN REP BAR left and right:
local REP_BAR_WIDTH = 256
local REP_BAR_HEIGHT = 18
local REP_BAR_STATUS_WIDTH = 508
local REP_BAR_STATUS_HEIGHT = 400
local repbar = CreateFrame("Frame", name, UIParent)
...
repbar.rightTexture = repbar.leftFrame:CreateTexture(nil, "BACKGROUND")
...
repbar.rightTexture:SetTexCoord(1, 0, 0, 1) --flip right
--end --MAIN REP BAR left and right

-- Create a font string for the custom text
local customRepText = repbar:CreateFontString(nil, "OVERLAY")
customRepText:SetFont("Fonts\\FRIZQT__.TTF", 12, "OUTLINE")
customRepText:SetPoint("CENTER", ReputationWatchBar, "CENTER", 0, -59)
customRepText:SetJustifyH("CENTER")

-- Function to update the custom text with the current faction name
local function UpdateCustomReputationText()
    local name, reaction, min, max, value = GetWatchedFactionInfo()
    if name then
        customRepText:SetText(name)
        repbar:Show()
    else
...
--[[
works perfectly, just need standing numbers as well:
-- Hide the original ReputationWatchBar
ReputationWatchBar:SetAlpha(0)
